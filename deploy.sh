...
http --session=session --follow "${BASE_URL}/terraform/modules/v1/A/B/C/versions"
echo "    The api returns 404 when there are no versions"
http --session=session --follow "${BASE_URL}/terraform/modules/v1/X/X/X/versions"
####

####
echo "When we request to download a version of a modules"
echo "   The API returns the X-TERRAFORM-GET header"
http --session=session --follow "${BASE_URL}/terraform/modules/v1/A/B/C/1.0.0/download"
